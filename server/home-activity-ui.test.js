...
test('Battle Friend доод navigation-ийн голд өргөгдсөн үндсэн action байна', () => {
  ...
  const board = html.indexOf('id="navBoard"')
  assert.ok(push > -1 && push < squat && squat < board && board < battle)
  assert.match(html, /id="navBattle"[^>]*aria-label="Battle Friend"/)
  assert.match(html, /class="navBattleOrb"/)
  assert.match(html, /assets\/figma\/nav-push\.svg/)
  assert.match(html, /assets\/figma\/nav-squat\.svg/)
  assert.match(html, /#mainNav #navBattle\{flex:none;width:65px;height:65px;min-height:65px;[^}]*border-radius:38px/)
  assert.match(html, /\.navBattleOrb\{width:20px;height:20px;[^}]*background:transparent/)
  assert.match(html, /#mainNav #navBattle\{[^}]*background:linear-gradient\(180deg,rgba\(249,115,22,\.2\),rgba\(249,115,22,0\)\)/)
